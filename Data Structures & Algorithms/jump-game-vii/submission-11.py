class Solution:
    def canReach(self, s: str, minJump: int, maxJump: int) -> bool:
        ...
        k = maxJump + 1
        reachable = sum(dp[k - maxJump: k - minJump + 1]) # keep a count of how many values are true, if this counter hits zero then it means that position cannot be reached
        if maxJump == minJump:
