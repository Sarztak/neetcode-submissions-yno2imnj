class Solution:
    def search(self, nums: List[int], target: int) -> bool:
        """
        I spend a lot of time thinking that there is a log(n) solution but there isn't
        Having duplicate elements breaks the conditions required for finding the minimum element
        in log(n) time complexity so this case at worst will degenerate into O(n) when all the elements are same 
        So I had to add a extra cases to handle the ambiguity when arr[lo] == arr[mid] == arr[hi] 
        """
